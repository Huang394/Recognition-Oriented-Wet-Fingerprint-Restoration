...
class ConvIR(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.FAM2(z, z2) #concat Conv3x3
        res2 = self.Encoder[1](z)
        # 64
        z = self.feat_extract[2](res2) #Conv3x3
        z = self.FAM1(z, z4) #concat Conv3x3
        z = self.Encoder[2](z)

        z = self.Decoder[0](z)
        z_ = self.ConvsOut[0](z)
        # 128
        z = self.feat_extract[3](z) #Conv4x4
        ...
        # 256
        z = self.feat_extract[4](z) #Conv4x4
        outputs.append(z_+x_2)

        z = torch.cat([z, res1], dim=1)
        z = self.Convs[1](z)
        z = self.Decoder[2](z)
        z = self.feat_extract[5](z) #Conv3x3
        outputs.append(z+x)
