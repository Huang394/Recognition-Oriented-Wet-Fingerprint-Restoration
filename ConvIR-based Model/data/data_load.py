...
def test_dataloader(path, data, batch_size=1, num_workers=0):
    ...
    dataloader = DataLoader(
        DeblurDataset(image_dir, data, is_test=True, transform=transform),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        pin_memory=True
...
def valid_dataloader(path, data, batch_size=1, num_workers=0):
    ...
    dataloader = DataLoader(
        DeblurDataset(os.path.join(path, 'test'), data, is_test=True, transform=transform),
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers
    )
...
class DeblurDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.data == 'ITS':
            ...
            #label = Image.open(os.path.join(self.image_dir, 'hazy', self.image_list[idx]))
        ...
        image = self.pad_image_to_256(image)
        label = self.pad_image_to_256(label)
    ...
    def pad_image_to_256(self, image):
        desired_size = 256
        old_size = image.size  # old_size[0] is in (width, height) format
    
        new_image = Image.new("RGB", (desired_size, desired_size), (0, 0, 0))
        new_image.paste(image, ((desired_size - old_size[0]) // 2,
